...
export const testCreateInstorePaymentIntent = () => {
  describe('test create instore payment intent', () => {
    let shop1: any;
    let shop2: any;
    ...
    // const shippingAddress = {  // allow null, optional field
    //   name: 'shipping name',
    //   phone: '9090909090',
    //   postalCode: '700000',
    //   state: 'Tan Binh',
    ...
    describe('create instore payment intent fail', () => {
      it('create instore payment intent requires authenticated user', async () => {
        const res = await request(app)
        .post(`/api/v1/payments/instore-order/stripe/intent`)
        .send(checkoutOrder);

        expect(res.statusCode).toEqual(401);
      });

      it('create instore payment intent fail due to missed required nameId', async () => {
        const res = await request(app)
        .post(`/api/v1/payments/instore-order/stripe/intent`)
        .set('Authorization', `Bearer ${userToken}`)
        .send({
      ...
      it('create instore payment intent fail due to missed required amount', async () => {
        const res = await request(app)
        .post(`/api/v1/payments/instore-order/stripe/intent`)
        .set('Authorization', `Bearer ${userToken}`)
        .send({
      ...
      it('create instore payment intent fail due to missed required orderDetails', async () => {
        const res = await request(app)
        .post(`/api/v1/payments/instore-order/stripe/intent`)
        .set('Authorization', `Bearer ${userToken}`)
        .send({
      ...
      it('create instore payment intent fail due to missed required totalAmount', async () => {
        const res = await request(app)
        .post(`/api/v1/payments/instore-order/stripe/intent`)
        .set('Authorization', `Bearer ${userToken}`)
        .send({
      ...
      it('create instore payment intent fail due to missed shipping address', async () => {
        const res = await request(app)
        .post(`/api/v1/payments/instore-order/stripe/intent`)
        .set('Authorization', `Bearer ${userToken}`)
        .send(checkoutOrder);
        ...
        expect(res.statusCode).toEqual(400);
        expect(res.body.error.message).toBe(InstoreOrderErrorMessageEnum.REQUIRED_SHIPPING_ADDRESS);
      });

    });

    describe('create instore payment intent fail due to conflict', () => {
      beforeEach(async () => {
      ...
      it('create instore payment intent fail due order was canceled', async () => {
      ...
      it('create instore payment intent fail due order items has error', async () => {
        await ProductDbModel.update({

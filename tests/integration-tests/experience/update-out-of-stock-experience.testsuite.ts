...
export const testUpdateOutOfStockExperience = () =>
  describe('Update Out Of Stock', () => {
    ...
    describe('Update Successfull: Update out of stock experience successfull', () => {
      it('should get return updated quatity is 0', async () => {

        const res = await request(app)
          .patch(`/api/v1/experiences/${experienceId}/out-of-stock`)
          .set('Authorization', `Bearer ${userToken}`)
        ...
        expect(experienceRes.body.data.quantity).toBe(0);
      });
    });

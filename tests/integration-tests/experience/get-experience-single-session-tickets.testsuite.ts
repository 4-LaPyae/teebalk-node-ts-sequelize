...
import { IExperienceSingleSessionTickets } from "../../../src/services";
...
export const getExperienceSingleSessionTicketsTest = () =>
    describe('Get Experience single Session Tickets Test', () => {
        let experience: any;
        ...
        it('when experience published should return 200', async () => {
            ...
            const res = await request(app)
                .get(`/api/v1/experiences/${experience.nameId}/${session.id}`)
                .set('Authorization', `Bearer ${userToken}`);

            const experienceData = res.body.data as IExperienceSingleSessionTickets;

            expect(res.statusCode).toEqual(200);
            expect(experienceData.shop).not.toBeUndefined();
            expect(experienceData.session).not.toBeUndefined();
            expect(experienceData.session).not.toBeUndefined();
            expect(experienceData.session.tickets[0].quantity).toEqual(2);

        });

        it('when session not belong to experience should return 404', async () => {

            const res = await request(app)
            .get(`/api/v1/experiences/${experience.nameId}/${-1}`)
            .set('Authorization', `Bearer ${userToken}`);

            expect(res.statusCode).toEqual(404);
        });

        it('when un-authentication should return 200', async ()=>{

...
export const testMoveToCart = () =>
  describe('CART: moveToCart', () => {
    ...
    it('should get return add cart item', async () => {
      ...
      const res = await request(app)
        .patch(`/api/v1/cart/${cart.id}/add-to-cart`)
        .set('Authorization', `Bearer ${userToken}`)
        .send(cartData);
      expect(res.statusCode).toEqual(200);
      expect(res.body.data).not.toBeUndefined();
      expect(res.body.data.status).toEqual(CartStatusEnum.IN_PROGRESS);
      expect(res.body.data.id).toEqual(cart.id);
    });

    it('should get return error 409', async () => {
      const res = await request(app)
        .patch(`/api/v1/cart/${cart.id}/add-to-cart`)
        .set('Authorization', `Bearer ${userToken}`);
    ...
    it('should get return error 400', async () => {
      const res = await request(app)
        .patch(`/api/v1/cart/0/add-to-cart`)
        .set('Authorization', `Bearer ${userToken}`);

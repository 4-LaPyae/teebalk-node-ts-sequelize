...
export const validateShopppingCart = () =>
  describe('CART: Validate items in cart', () => {
    ...
    let product: any, productColor: any, productCustomParameter: any;
    ...
    beforeAll(async () => {
      ...
      await ProductParameterSetDbModel.create({
        productId: product.id,
    ...
    it('should get return true', async () => {
      const products = {
        products: [
          {
            productId: product.id,
      ...
      const res = await request(app)
        .post(`/api/v1/cart/validate-cart-items`)

...
jest.mock('../../../src/services', () => {
  let cartService = {
    deleteCartItemById: jest.fn(() => Promise.resolve(true))
  };
...
describe('Controller:Cart:AddProductToCart', () => {
  describe('Cart:AddProductToCart', () => {
    ...
    describe('DeleteItemInCart: Check return status when deleted successfully ', () => {
      it('should return true', async () => {
        const result = await cartController.deleteCartItem(1);
        expect(result).toBe(true);
      });
    });

    describe('DeleteItemInCart: Error missing userId', () => {
      it('should return ERROR message', async () => {
        try {
          await cartController.deleteCartItem(0);
        } catch (error) {
    ...
    describe('DeleteItemInCart: Error missing when deleteting', () => {
      it('should return ERROR message', async () => {
        try {
          cartService.deleteCartItemById = jest.fn(() => {
            throw new Error();
          });

...
jest.mock('../../../src/services', () => {
  const cartService = {
    addToCart: jest.fn(() => Promise.resolve(mockData)),
...
describe('Controller:Cart:AddProductToCart', () => {
  describe('Cart:AddProductToCart', () => {
    ...
    describe('AddProductToCart: Check return cart item', () => {
      it('should return equal the mock data', async () => {
        const result = await cartController.addToCart(1, mockCart);
        expect(result).toBe(mockCartItemDetail);
      });
    });

    describe('AddProductToCart: Error missing userId', () => {
      it('should return ERROR message', async () => {
        try {
          await cartController.addToCart(0, mockCart);
        } catch (error) {
    ...
    describe('AddProductToCart: Error missing cart', () => {
      it('should return error message', async () => {
        try {
          await cartController.addToCart(1, null as any);
        } catch (error) {

...
describe('Unitest:Service:Cart:GetList:ProductWithoutParameters', () => {
  describe('Cart:GetList', () => {
    ...
    describe('GetList: ValidateProductParametersInCart', () => {
      ...
      describe('Case 1', () => {
        it('should return list of cart items which does not have error', async () => {
          const mockCart = [
            {
              id: 1,
              productId: 1,
          ...
          expect(cartItem.rows[0].errors.length).toEqual(0);
        });
      });
      ...
      describe('Case 2', () => {
        it('should return equal the mock data', async () => {
          ...
          expect(cartItem.rows[0].errors[0].value).toStrictEqual(PurchaseItemErrorMessageEnum.DUPLICATED_PARAMETER);
        });
      });

      describe('Case 3', () => {
        it('should return list with warning AllParametersAreRemoved', async () => {
          const mockCart = [
            {
              id: 1,
              productId: 1,
          ...
          const cartItem = await cartService.getInProgressCartItemsList(1, false);
          expect(cartItem.count).toBe(1);
          expect(cartItem.rows[0].errors[0].value).toStrictEqual(PurchaseItemErrorMessageEnum.ALL_PARAMETERS_ARE_REMOVED);
        });
      });
    });
  });

...
jest.mock('../../../src/dal', () => {
  const productRepository = {
    count: jest.fn(() => Promise.resolve(mockData))
  };
...
describe('Service:Product:Count', () => {
  describe('Product:Count', () => {
    ...
    describe('Count:Check name id of return product', () => {
      it('should return W4kfiygStYTwTFtQ8WN9yidUoBqlwV', async () => {
        const result = await productService.countOnlineProduct(1);
        const expectResult = {
          publishedItems: 15,
          unpublishedItems: 15
        };

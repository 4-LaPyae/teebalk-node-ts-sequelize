...
const mockDataHighlightPoints = [
  {
    id: 1,
    nameId: 'Artisan',
    value: 1,
    backgroundImage: '/assets/images/highlight-point/artisan.png',
    type: 'ethicality_level',
    createdAt: '2021-07-02 12:42:34',
    updatedAt: '2021-07-02 12:42:34'
  }
];

const mockDataProductTransparencyRepository = {
...
jest.mock('../../../src/dal', () => {
  const productRepository = {
    ...
    createProduct: jest.fn(() => Promise.resolve(mockData))
  };

  const highlightPointRepository = {
    bulkCreate: jest.fn(() => Promise.resolve(mockDataHighlightPoints)),
    getAllByTypeAndIds: jest.fn(() => Promise.resolve([]))
  };

  const productHighlightPointRepository = {
  ...
  const productTransparencyRepository = {
    create: jest.fn(() => Promise.resolve(mockDataProductTransparencyRepository))
  };
  ...
  return {
    ...
    ProductHighlightPointRepository: jest.fn(() => productHighlightPointRepository),
  ...
  describe('Product:Create', () => {
    ...
    describe('GetProductByNameId:Check name id of return product', () => {
      it('should return W4kfiygStYTwTFtQ8WN9yidUoBqlwV', async () => {
        const result = await productService.create(1, 1, mockProduct);

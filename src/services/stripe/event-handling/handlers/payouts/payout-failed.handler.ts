...
const log = new Logger('SRV:Stripe:PayoutFailedEventHandler');

export class PayoutFailedEventHandler extends StripeEventHandlerBase {
  get eventType() {
  ...
  async handle(event: Stripe.Event, transaction?: Transaction) {
    ...
    log.verbose(`Payout with ID=${payout.id} got status ${status}`);
  }
}
